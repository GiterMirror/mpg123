 ...
 *   3 10/24/00 Added REAL_IS_FIXED option.                            GbG  *
 ****************************************************************************/
...
#if defined(REAL_IS_FIXED)


/*
 * Function to support fixed-point version of mpg123 decoder
 * in the absence of official long long 64-bit integer type;
 * we presume that we're on the appropriate conditional
 * compilation path, and that "real" has been #defined or
 * typedef-ed appropriately, and that REAL_RADIX is #defined
 * to be the desired number of binary points.
 */

 real
 real_mul(real x, real y)
 {
#if defined(_MPFARM)
	unsigned int		xyhi, xylo;
	
	_asm(-100, " smull %0,%1,%2,%3",
		__obj_assign(__reg_gen(xylo)),
		__obj_assign(__reg_gen(xyhi)),				
		__reg_gen(x),					/* Have to force x away */
		__reg_gen(y));					/* from regs for xylo, xyhi. */
	x = x;								/* Gratuitous life extension should work... */
#elif defined(_MPF386)
	unsigned int		xyhi;

	_asm(-100, " imul.l %0",
		__reg_gen(y),
		__obj_modify(__reg_eax(x)),		/* x overwritten with LSL of product */
		__obj_assign(__reg_edx(xyhi)));
#define xylo ((unsigned int) x)
#else
#error no real_mul() for this target yet
#endif

	return (xylo >> REAL_RADIX) |
			(xyhi << (8 * sizeof(int) - REAL_RADIX));
}
...
void init_layer3(int down_sample_sblimit)
{
  ...
  for(i=0;i<18;i++)
  {
	...
	a = sin( M_PI / 72.0 * (double) (2*(i+0) +1));
	b = cos ( M_PI * (double) (2*(i+0) +19) / 72.0 );
	c = (0.5 * a / b);

    win[0][i]    = win[1][i]    = DOUBLE_TO_REAL(c);

	a = sin( M_PI / 72.0 * (double) (2*(i+18)+1));
	b = cos ( M_PI * (double) (2*(i+18) +19) / 72.0 );
	c = (0.5 * a / b);

    win[0][i+18] = win[3][i+18] = DOUBLE_TO_REAL(c);

  }
  for(i=0;i<6;i++)
  {

	a = cos ( M_PI * (double) (2*(i+18)+19) / 72.0 );
	b = (0.5 / a); 
    win[1][i+18] = DOUBLE_TO_REAL(b);
	a = cos ( M_PI * (double) (2*(i+12)+19) / 72.0 ); 
	b = (0.5 / a);
    win[3][i+12] = DOUBLE_TO_REAL(b);
	a = sin( M_PI / 24.0 * (double) (2*i+13) );
	b = cos ( M_PI * (double) (2*(i+24)+19) / 72.0 );
	c = (0.5 * a / b);
    win[1][i+24] = DOUBLE_TO_REAL(c);
    win[1][i+30] = win[3][i] = DOUBLE_TO_REAL(0.0);
	a = sin( M_PI / 24.0 * (double) (2*i+1));
	b = cos ( M_PI * (double) (2*(i+6 )+19) / 72.0 );
	c = (0.5 * a / b);
    win[3][i+6 ] = DOUBLE_TO_REAL(c);
  }
/*
  for(i=0;i<9;i++)
...
*/
  ...
  {
	a = sin( M_PI / 24.0 * (double) (2*i+1));
	b = cos ( M_PI * (double) (2*i+7) / 24.0 );
	c = (0.5 * a / b);
    win[2][i]  = DOUBLE_TO_REAL(c);
/*
    for(j=0;j<6;j++)
 ...
 */
  ...
  if(part2remain > 0)
    ...
    printf("layer3: Can not rewind stream by %d bits!\n",-part2remain); 
	return 0;
  }
 ...
 */
static void III_i_stereo(real xr[SBLIMIT][SSLIMIT],int *scalefac,
...
int do_layer3(struct frame *fr,unsigned char *pcm_sample,int *pcm_point)
{
  ...
  for (gr=0;gr<granules;gr++) 
  {
	...
	fprintf(stderr,"Not supported\n");
      ...
      if(i_stereo)
        III_i_stereo(hybridIn[0],scalefacs[1],gr_info,sfreq,ms_stereo,fr->lsf);
